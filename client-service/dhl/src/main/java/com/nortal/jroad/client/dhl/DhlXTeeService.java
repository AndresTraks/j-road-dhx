...
import com.nortal.jroad.client.dhl.types.ee.riik.xrd.dhl.producers.producer.dhl.GetSendStatusV2ResponseTypeUnencoded;
...
public interface DhlXTeeService {
     ...
     * @param recipients
     * @param sender
     * @param decContainerCallback - optional callBack implementation to be called by the service method when the document is composed, allowing to change content of
     *            <i>"decContainer"</i> as desired.
     * @return dhl_Id's of the sent document
     */
    Set<String> sendDocuments(Collection<ContentToSend> contentsToSend, DecContainerDocument.DecContainer.Transport.DecRecipient[] recipients, DecContainerDocument.DecContainer.Transport.DecSender sender, SendDocumentsDecContainerCallback decContainerCallback,
            SendDocumentsRequestCallback requestCallback);
     ...
     */
    Map<String, List<Map<String, Serializable>>> getSendStatuses(Set<String> ids);
    ...
    /**
     * Allows to mark documents received lik {@link #markDocumentsReceivedV2(Collection)}, but also enables to add some additional information.<br>
    ...
    interface SendDocumentsRequestCallback {
        void doWithRequest(SendDocumentsV4RequestType request);
    }

    /**
     * Callback interface, that can be used to alter <i>"DecContainer"</i> element after it has been composed <br>
     * (just before attaching it to request body and making the service call)
     */
    interface SendDocumentsDecContainerCallback {
        void doWithDocument(DecContainerDocument decContainerDocument);
    }
