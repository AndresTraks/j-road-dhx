...
public class StandardXRoadConsumer extends WebServiceGatewaySupport implements XRoadConsumer {
  ...
  private <I, O> XRoadMessage<O> sendRealRequest(XRoadMessage<I> input,
    ...
    try {
      ...
      Set<XmlObject> allObjects = XmlBeansUtil.getAllObjects((XmlObject) input.getContent());
      for (XmlObject attachmentObj : allObjects) {

        // Introspect all methods, and find the ones that were generated during instrumentation
        List<Method> swaRefGetters = XmlBeansUtil.getSwaRefGetters(attachmentObj);
        for (Method method : swaRefGetters) {
      ...
      String curDataKey = xroadServiceConfiguration.getWsdlDatabase().toLowerCase()
          + xroadServiceConfiguration.getMethod().toLowerCase();
      XmlBeansXRoadMetadata curdata = metadata.get(curDataKey);
